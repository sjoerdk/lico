...
def test_lico():
    ...
    output_list = CSVFile.init_from_table(table=process(input_list,
...
def test_missing_parameter():
    ...
    with pytest.raises(RowProcessError) as e:
        process(generate_table(fieldnames=['patient', 'missingdate']),
                              Concatenate(columns=['patient', 'date']),
...
def test_read_table_set_column_names():
    ...
    with pytest.raises(ValueError):
        CSVFile.init_from_path(RESOURCE_PATH / "a_csv_table_file",
...
def a_csv_table_file(tmp_path) -> CSVFile:
    """A csv file on disk"""
    file = CSVFile.init_from_table(path=tmp_path / "a_csv_table_file.csv",
                                   table=generate_table(fieldnames=['field1',
                                                                    'field2']))
    file.save()
    return file
...
def test_task(a_csv_table_file, tmp_path, caplog):
    ...
    task = Task(input_file=a_csv_table_file,
